...
class RDDLDecisionWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.full_logs.append({"step_type": "RESET", "state": obs.copy()})

        if not self.get_active_stations(obs):
            obs, _, terminated, truncated, info = self._skip_forward(
                0, False, False, info
            )
    ...
    def step(self, action):
        ...
        if not terminated and not truncated:
            obs, extra_reward, terminated, truncated, info = self._skip_forward(
            ...
            if timer == global_timer:
                if state == self.TRAIN_IN_ROUTE:
                    ...
                    if occupant is None:
                        ...
                        if (
                            occupant_state == self.TRAIN_ACTIVE
                            and occupant_timer == global_timer
                            and not station_has_queue[station]
                        ):
                            active_set.add(station)
